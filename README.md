...
📝 更新日志
v2.2.2 (2026-06-03)
🐛 重大修复：修复 /聊天检测 和 /重置检测 命令输入时报错"必要参数缺失"的问题。
问题原因：AstrBot 命令解析器将方法签名中的 *args 参数解释为必需参数，当用户不提供参数时会报错。
修复方案：移除 manual_check、reset_check_status 和 on_message 方法中未使用的 *args 参数。
影响范围：现在输入 /聊天检测 和 /重置检测 时不再需要任何参数，可直接使用。
v2.1.1 (2026-01-27)
